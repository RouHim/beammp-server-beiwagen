...
pub fn get_to_remove(local_list: &HashMap<u64, Resource>, remote_list: &HashMap<u64, Resource>) -> Vec<Resource> {
    let mut deleted_entries: Vec<Resource> = local_list
        .iter()
        .filter(|(key, val)| !remote_list.contains_key(key))
        .map(|(key, val)| val.clone())
        .collect();

    let mut outdated_entries: Vec<Resource> = remote_list
        .iter()
        .filter(|(key, val)| should_delete_outdated(val))
        .map(|(key, val)| val.clone())
        .collect();

    let mut unsupported_entries: Vec<Resource> = remote_list
        .iter()
        .filter(|(key, val)| should_delete_unsupported(val))
        .map(|(key, val)| val.clone())
        .collect();
